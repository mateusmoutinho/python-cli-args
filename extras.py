...
def is_a_flag(
    ...
    #len of comparation is > 1 means its a flag 
    #the len comparation is to not return true for "-" args
...
def format_args(args:list,consider_first:bool,case_sensitive=bool,convert_numbers=bool):
    ...
    convert_numbers: convert numbers in float or int if possible'''
     #split the args removing the first ocurrence, if not consider first
